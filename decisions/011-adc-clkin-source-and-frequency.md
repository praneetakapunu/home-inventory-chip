...
### Current evidence snapshot (as of harness HEAD f6d7178)
Running:
```bash
...
- Source: `home-inventory-chip-openmpw/docs/source/adc_clocking_plan.md:18-26` — defines the allowed v1 options (A: board oscillator, B: SoC-driven `adc_clkin`), and explicitly states under Option A that the SoC does **not** route `adc_clkin`.
- Source: `home-inventory-chip-openmpw/docs/source/adc_clocking_plan.md:27-37` — defines the “confirmed” evidence checklist (decision, source, frequency, reset presence, scope point, etc.) and requires committed evidence.
- Source: `home-inventory-chip-openmpw/docs/source/adc_clocking_plan.md:59-62` — status remains `Decision: TBD` / `Expected CLKIN frequency: TBD`.
