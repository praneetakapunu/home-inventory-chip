...
## Definition: “~5 g effective resolution”

This project target is *system-level*, not an ADC LSB claim. For v1, we will treat a channel as meeting the **~5 g effective resolution** goal if all of the following are true after tare + the nominal digital filter, under a controlled bench setup:

1) **Noise (short-term):** with a constant load, the reported weight has
   - RMS noise ≤ **2 g** over a **10 s** window, and
   - peak-to-peak noise ≤ **±5 g** over the same window.

2) **Step detectability:** a **+5 g** and **−5 g** step applied to the pad produces the correct add/remove event with
...
3) **Drift (medium-term):** with constant load and stable temperature, the reported weight drifts by
   - ≤ **±5 g** over **30 minutes** after an initial 2-minute settling period.
...
- Apply known step weights (5 g, 10 g) and measure detection latency + false positives.
